---
name: coding-agent
description: Delega a escrita completa da task definida no SDD para um agente background (Codex/Claude Code) em bash. Execute durante a Fase 3 (Tasks) quando uma task longa exigir extensa criação de código base. Não use para ajustes rápidos. PTY Requerido para Codex.
metadata:
  {
    "openclaw":
      {
